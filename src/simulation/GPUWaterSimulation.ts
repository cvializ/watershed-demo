...
export class GPUWaterSimulation {
    ...
    /**
     * Add rain at a specific position
     */
    public addRain(position: THREE.Vector2, amount: number = 0.1, radius: number = 5): void {
        const texture = this.currentHeightBuffer === 0 ? 
            this.waterHeightTexture : this.outputHeightTexture;
        
        // Convert world position to texture coordinates
        const u = (position.x + 1.0) * 0.5; // Assuming terrain centered at origin
        const v = (position.y + 1.0) * 0.5;
        
        // Add rain in a circular region
        for (let i = -radius; i <= radius; i++) {
            for (let j = -radius; j <= radius; j++) {
                const x = Math.floor(u * this.width + i);
                const y = Math.floor(v * this.height + j);
